...
# getwd()
data1 <- readRDS(here("export", "data_clean.rds"))
# data1
```
...
group <- read.csv(here("export", "ParameterGrouping.csv"))
data <- data3
  ...
  tryCatch(
    {
      ...
      cvr <- metafor::rma.mv(yi = effect_size_CVR, V = sample_variance_CVR, 
                             random = list(~ 1 | strain_name, ~ 1 | production_center, ~ 1 | err), control = list(optimizer = "optim", optmethod = "Nelder-Mead", maxit = 1000), verbose = F, data = results)
