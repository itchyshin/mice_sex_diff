...
clean_raw_data <- function(mydata) {
  mydata %>%
    ...
    mutate(center_per_trait = length(unique(production_center, na.rm = TRUE))) %>%
    ungroup() %>% 
...
data_raw %>% 
  clean_raw_data() %>% 
  saveRDS("export/data_clean.rds")
```

For analysis we load the RDS created above:
  ...
  tryCatch(
    {
      ...
      results_alltraits_grouping[t, 2] <- cvr$b
      results_alltraits_grouping[t, 3] <- cvr$ci.lb
      results_alltraits_grouping[t, 4] <- cvr$ci.ub
      results_alltraits_grouping[t, 5] <- cvr$se
      ...
      results_alltraits_grouping[t, 6] <- cv$b
      results_alltraits_grouping[t, 7] <- cv$ci.lb
      results_alltraits_grouping[t, 8] <- cv$ci.ub
      results_alltraits_grouping[t, 9] <- cv$se
      ...
      means <- metafor::rma.mv(yi = effect_size_RR, V = sample_variance_RR, 
                               ...
                               control = list(optimizer = "optim", optmethod = "Nelder-Mead", 
                                              maxit = 1000), 
                               data = results)
      results_alltraits_grouping[t, 10] <- means$b
      results_alltraits_grouping[t, 11] <- means$ci.lb
      results_alltraits_grouping[t, 12] <- means$ci.ub
      results_alltraits_grouping[t, 13] <- means$se


      results_alltraits_grouping[t, 14] <- means$k
...
# trait_meta_results <- write.csv(results_alltraits_grouping, file = "export/trait_meta_results.csv")
```
...
results_alltraits_grouping$parameter_group <- data$parameter_group[match(results_alltraits_grouping$id, data$id)]
results_alltraits_grouping$procedure <- data$procedure_name[match(results_alltraits_grouping$id, data$id)]

results_alltraits_grouping$GroupingTerm <- procedures$GroupingTerm[match(results_alltraits_grouping$procedure, procedures$procedure)]
results_alltraits_grouping$parameter_name <- data$parameter_name[match(results_alltraits_grouping$id, data$id)]

meta1 <- results_alltraits_grouping
...
meta_problem <- results_alltraits_grouping[ (results_alltraits_grouping$id %in% c(84, 144, 158, 160, 161, 162, 163, 165, 166, 167, 168, 221, 222, 231)), ]
```
