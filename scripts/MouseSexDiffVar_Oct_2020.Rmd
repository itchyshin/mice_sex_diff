---
title: 'Sexual dimorphism in trait variability and its eco-evolutionary
...
output:
  ...
  bookdown::word_document2:
    ...
    number_sections: true
    theme: flatly
subtitle: Electronic Supporting File (doi:10.5281/zenodo.4146948)
...
```{r FigG1, echo = FALSE, fig.width = 4, fig.height = 8, fig.cap="Workflow of data processing and meta-analysis [Figure 3 in manuscipt]"}
...
plot3.male.meta.CVR.b <- rbind(plot3.male.meta.CVR.b, add.row.hearing) #bind_rows
...
lnRR.ci <- overall3.male.sig           %>%
           ...
           mutate(ci.low = lnRR_lower, 
                 ci.high = lnRR_upper)
overall4.male.sig <- rbind(lnCVR.ci, lnRR.ci) %>%  #bind_rows
                     select(GroupingTerm, 
                            parameter, 
                            value, 
                            ci.low, 
...
plot3.female.meta.RR.all <- plot3.female.meta.RR.all %>% 
                            mutate(GroupingTerm = "All")
# Combine with separate grouping term results
plot3.female.meta.CVR <- bind_rows(plot3.female.meta.CVR, plot3.female.meta.CVR.all) #bind_rows
...
plot3.female.meta.CVR.b <- rbind(plot3.female.meta.CVR.b, add.row.hearing) #bind_rows
...
lnRR.ci <- overall3.female.sig %>%
  filter(parameter == "lnRR") %>%
  mutate(ci.low = lnRR_lower, ci.high = lnRR_upper)
overall4.female.sig <- rbind(lnCVR.ci, lnRR.ci) %>%  #bind_rows
...
overall4.male.sigS <- rbind(lnCVR.ci, lnVR.ci, lnRR.ci) %>% select(GroupingTerm, parameter, value, ci.low, ci.high) #bind_rows
