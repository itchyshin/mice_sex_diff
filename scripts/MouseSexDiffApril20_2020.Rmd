...
author: Susanne Zajitschek,  Felix Zajitschek, Russell Bonduriansky, Robert  Brooks,
  Will Cornwell, Daniel Falster, Malgortaza Lagisz, Jeremy Mason, Daniel Noble, Alistair
  Senior & Shinichi Nakagawa
date: "April 2020"
output:
  html_document:
    code_download: yes
...
library(ggplot2)
library(here)
# additional packages for incorporating male / female symbols in plot 4
library(png)
library(grid)
```
...
This function groups animals from the same strain and institution together. This is done for each trait separately, and only for traits that have been measured in both sexes. Any group containing fewer than 5 individuals is excluded.
...
# Will calculate population stats and exclude data that is irrelevant and not useful, it will also just convert the data straight away to wide format as opposed to long, which I think is making things more complicated when effect sizes are calculated. 

calculate_population_stats_new <- function(mydata, min_individuals = 5){
    mydata %>% 
    group_by(population,
             production_center,  
             strain_name,  
...
#This function should mostly replace the entire function below, but I am still not quite sure what is happening in places. Adding all effect sizes together (maintaining the naming), err seems to be added to each trait? That needs to be added.It will make use of escalc which has the correct equations for effect size calculations.
create_meta_analysis_effect_sizes_new <- function(mydata, measure = c("CVR", "VR", "ROM")){
  for(i in 1:length(measure)){
    mydata <- metafor::escalc(m1i  = female_x_bar, 
                              m2i  = male_x_bar, 
                              sd1i = female_x_sd, 
                              sd2i = male_x_sd, 
                              n1i  = female_n_ind, 
...
## Can delete all the below ###### The above function does same thing, but uses simplified dataset and also reorganises it so no need for input dataframe. This also means we can remove the next section with Al's function as escalc has these built in anyway.
...
```{r clean, eval=FALSE, include=FALSE}
...
### 1.1: Loop: Meta-analyses on all traits
...
for (t in 1:n) {
  tryCatch(
    ...
    #NOTE: results and results 2 should now be equivalent and produce the same thing except that results 2 has the columns added in with raw effect sizes, which I think is wise if we can keep them. Otherwise, the naming should be the same. I've checked quite a few t's and all looks the same. results 2 just has more columns.
      ...
      results2 <- data %>%
       data_subset_parameterid_individual_by_age(t) %>%
       calculate_population_stats_new() %>%
       create_meta_analysis_effect_sizes_new() %>%
...
### 1.2: Merging datasets & removal of non-converged traits
...
### 2.1: Dealing with Correlated Parameters, preparation
...
### 2.2: Collapsing and merging correlated parameters, preparation for Table S2
...
### 2.3: Meta-analyses on correlated (sub-)traits, using `robumeta` 
...
# Here we apply an additional Function to collect the outcomes of the 'mini-meta-analysis' that has ondensed our non-independent traits. Values from our second-order meta-analysis using robu-meta are extracted
...
#### 3.1.2: Re-structuring the data for each grouping term
...
Behaviour <- overall1 %>% 
  ...
  mutate(
    lnCVR = .[[4]][[1]]$b, lnCVR_lower = .[[4]][[1]]$ci.lb, lnCVR_upper = .[[4]][[1]]$ci.ub, lnCVR_se = .[[4]][[1]]$se,     lnCVR_I2 = .[[4]][[1]]$I2,
...
All <- overall_all1 %>% 
  mutate(
    lnCVR = .[[2]][[1]]$b, lnCVR_lower = .[[2]][[1]]$ci.lb, lnCVR_upper = .[[2]][[1]]$ci.ub, lnCVR_se = .[[2]][[1]]$se,     lnCVR_I2 = .[[2]][[1]]$I2,
...
## 1. Preparation for Figure 4 
#### 1.1: Count data, based on First-order meta-analysis results
...
#### 1.2: Overall results of second order meta-analysis (Figure 4, Panel B)
#### 1.3: Re-structure data for plotting 
...
## 3. Preparation for Figure 5
#### 3.1: Preparing data for traits with CI not overlapping 0
...
plot3.male.meta.CVR.b <- as.data.frame(plot3.male.meta.CVR %>% group_by(GroupingTerm) %>%
  mutate(
    ...
    lnCVR_I2 = map_dbl(model_lnCVR, pluck("I2"))
  ))[, c(1, 4:8)]
add.row.hearing <- as.data.frame(t(c("Hearing", NA, NA, NA, NA, NA))) %>% setNames(names(plot3.male.meta.CVR.b))
...
plot3.male.meta.VR.b <- as.data.frame(plot3.male.meta.VR %>% group_by(GroupingTerm) %>%
  mutate(
    ...
    lnVR_I2 = map_dbl(model_lnVR, pluck("I2"))
  ))[, c(1, 4:8)]
...
plot3.male.meta.RR.b <- as.data.frame(plot3.male.meta.RR %>% group_by(GroupingTerm) %>%
  mutate(
    ...
    lnRR_I2 = map_dbl(model_lnRR, pluck("I2"))
  ))[, c(1, 4:8)]
...
add.row.hearing <- as.data.frame(t(c("Hearing", NA, NA, NA, NA,NA))) %>% setNames(names(plot3.female.meta.CVR.b))
...
#Joining the Parts:
  ...
  Fig5B <- annotate_figure(Fig5B,
                bottom= text_grob("Effect Size", color = "black", face = "plain", size = 14))  # SUSI NEW
...
## Supplemental Tables
#### 1. Supplemental Table S1: "Strains and Center Sample Sizes"
...
```{r Table1_prep}
  ...
  ST1 <- (cbind(data %>% group_by(production_center, strain_name) %>% count(biological_sample_id, sex) %>% count(sex) %>% print(n = Inf))) 
```
```{r Table1}
...
#### 2. Supplemental Table S2: "Numbers of correlated and uncorrelated traits"
...
#### 3. Supplemental Table S3: "Meta-analysis results"
...
```{r Table3}
    kable(metacombo, digits = 3) %>%
    kable_styling() %>%
    scroll_box(width = "100%", height = "200px")

```

#### 4. Supplemental Table S4: "Meta-analysis on Functional Trait Groups"
  SHINICHI
  This table summarizes our overall meta-analyses on the functional trait group level ("GroupingTerm"). Results for lnCVR, lnVR and lnRR and their respective upper and lower 95% CI's, standard error and I2 values are provided.
```{r Table4}
  kable(overall2, digits = 3) %>%
   kable_styling() %>%
  scroll_box(width = "100%", height = "200px")
```

#### 5. Supplemental Table S5: "Sex-biased effect sizes"
  SHINICHI
  This table provides some of the data plotted in Figures 5 and Figure S2. Here, we provide an overview over meta-analysis results performed on traits that were significantly biased to either sex. This table summarizes findings for both sexes and the respective functional trait groups.
...
```{r hetero_loop, message=FALSE, warning=FALSE, include=FALSE }
# Create dataframe to store results
...
```
#### 6. Supplemental Table S6: "Heterogeneity within groups"
  SHINICHI
  This table summarizes our findings on heterogeneity due to institutions and mouse strains. These results are based on meta-analyses on sigma^2 and errors for mouse strains and centers (Institutions), following the identical workflow from above. 
...
```{r}
  ...
  het <- heterog6 %>%
    ...
    rename(lnCVR = value_heteroCVR, lnCVR_lower = ci.low_heteroCVR, lnCVR_higher = ci.high_heteroCVR, 
         ...
         VR_lower = ci.l_heteroVR, RR_lower = ci.l_heteroRR, 
         CVR_higher = ci.h_heteroCVR, VR_higher = ci.h_heteroVR, RR_higher = ci.h_heteroRR) %>%
...
Similar to Figure 4 in the main article, this figure shows the numbers of either male (blue-green bars) or female (orange-red bars) biased traits  (Panel A) across functional groups, this time for lnCVR (left hand side), lnVR (middle) and lnRR (right hand side). Panel B shows effect sizes from separate meta-analysis for each functional group, and Panel C contains results of heterogeneity analyses. All three panels represent results evaluated across all traits (Phase 3, Figure 3). 

```{r FigS1, fig.height = 8, fig.width = 7}
...
### 3. Preparation for Figure S2
#### 3.1: Preparation for Figure S2 A
