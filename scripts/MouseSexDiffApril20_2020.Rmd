...
This step we have already done and provide a cleaned up file which is less computing intensive and which we have saved in a folder called `export`. However, the cvs is provided in case this is preferred to be attempted, following the steps below: [This code is currently excluded from showing in this html file, but can be viewed and enabled / run in the downloadable .rmd file. However, it requires the .gz file containing the raw data in the "export" folder. Unfortuntaltely, that file can not be provided via github, as is too large (274MB). It is freely accessible, uploaded on zenodo.org (https://zenodo.org/deposit/3759701)
...
### Supplemental Table 2: "Numbers of correlated and uncorrelated traits"
...
```
### Supplemental Table 3: "Meta-analysis results"
...
```
### Supplemental Table 4: "Meta-analysis on Functional Trait Groups"
SHINICHI
...
```{r Fig4}
Fig4 <- ggarrange(malebias_Fig2_alltraits, Metameta_Fig3_alltraits,  nrow = 2, align = "v", heights = c(9, 9), labels = c("A", "B"))
Fig4
```


## Preparation for Figure 5: Sex bias.
...
## Heterogeneity Table and Plot
### Supplemental Table 6: "Heterogeneity within groups"
SHINICHI
...
FigS1 <- ggarrange(malebias_FigS1_alltraits, Metameta_FigS1_alltraits, HeteroS1, nrow = 3, align = "v", heights = c(1, 1, 1), labels = c("A", "B", "C"))
FigS1
#ggsave("FigS1_OverallResults.pdf", plot = FigS1, width = 6, height = 7)
```

### Preparations for Figure S2 A
...
### Preparation for Figure S2 B (second-order meta-analysis)
#### Female Figure, significant and highly biased traits (Figure S2 B, left hand side panels)
Preparing data for traits with CI not overlapping 0: 
...
```
#### Male Figure, significant and highly biased traits (Figure S2 B, right hand side panels)
...
```{r FigS2, fig.height = 8, fig.width = 7}
library(ggpubr)
FigS2b <- ggarrange(Metameta_FigS2_female.sig, Metameta_FigS2_male.sig,
  ncol = 2, nrow = 1, widths = c(1, 1.30), heights = c(1, 1)
)
...
FigS2 <- ggarrange(malebias_FigS2_sigtraits, malebias_Fig2_over10, FigS2b, FigS2d, ncol = 1, nrow = 4, heights = c(2.4, 2.1, 2.1, 2), labels = c("A", " ", "B", " "))
FigS2
#ggsave("FigS2_Bias.pdf", plot = FigS2, width = 7, height = 8)
```
